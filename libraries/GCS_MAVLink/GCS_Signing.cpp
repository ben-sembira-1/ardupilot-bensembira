...
extern "C" {
    ...
    unsigned_accept_cb_t generate_accept_unsigned_callback(bool accept_all)
    {
        static bool accept_unsigned_callback(const mavlink_status_t *status, uint32_t msgId)
        {
            if (accept_all || status == mavlink_get_channel_status(MAVLINK_COMM_0)) {
        ...
        return accept_unsigned_callback;
    }
}
